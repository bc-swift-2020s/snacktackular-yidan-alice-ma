...
class Review {
    ...
    func saveData(spot: Spot, completed: @escaping (Bool) -> ()) {
        ...
        // Create the dictionary representating the data we want to save
        ...
        if self.documentID != "" {
            ...
            ref = db.collection("spots").document(spot.documentID).collection("reviews").addDocument(data: dataToSave) { error in
                if let error = error {
                    print("*** ERROR: updating document in spot \(spot.documentID) for new review documentID \(error.localizedDescription)")
                    completed(false)
                } else {
